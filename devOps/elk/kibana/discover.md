...
你可以在 Discover 页加载已保存的搜索，也可以用作 [visualizations](https://kibana.logstash.es/content/kibana/v5/visualize.md) 的基础。保存一个搜索，意味着同时保存下了搜索请求字符串和当前选择的索引模式。
...
关于索引模式的更多细节，请阅读稍后 [Setting 功能小节](https://kibana.logstash.es/content/kibana/v5/settings.html)。
...
1. 点击菜单栏右上角的 `Time Picker` ![img](images/2.jpg)。
...
2. 要添加正向过滤器，点击 `Positive Filter` 按钮 ![img](images/4.jpg)。这个会过滤掉在本字段不包含这个数据的文档。
3. 要添加反向过滤器，点击 `Negative Filter` 按钮 ![img](images/5.jpg)。这个会过滤掉在本字段包含这个数据的文档。

要从文档表格添加过滤器：

1. 点击表格第一列(通常都是时间)文档内容左侧的 `Expand` 按钮 ![img](images/6.jpg) 展开文档表格中的文档。每个字段名的右侧，有两个小按钮 —— 一个用来添加常规(正向)过滤器，一个用来添加反向过滤器。
2. 要添加正向过滤器，点击 `Positive Filter` 按钮 ![img](images/4.jpg)。这个会过滤掉在本字段不包含这个数据的文档。
3. 要添加反向过滤器，点击 `Negative Filter` 按钮 ![img](images/5.jpg)。这个会过滤掉在本字段包含这个数据的文档。
...
- 过滤器开关 ![img](images/8.png) 点击这个图标，可以在不移除过滤器的情况下关闭过滤条件。再次点击则重新打开。被禁用的过滤器是条纹状的灰色，要求包含(相当于 Kibana3 里的 must)的过滤条件显示为绿色，要求排除(相当于 Kibana3 里的 mustNot)的过滤条件显示为红色。

- 过滤器图钉 ![img](images/9.png) 点击这个图标*钉住*过滤器。被钉住的过滤器，可以横贯 Kibana 各个标签生效。比如在 *Visualize* 标签页钉住一个过滤器，然后切换到 *Discover* 或者 *Dashboard* 标签页，过滤器依然还在。注意：如果你钉住了过滤器，然后发现检索结果为空，注意查看当前标签页的索引模式是不是跟过滤器匹配。

- 过滤器反转 ![img](images/10.png) 点击这个图标*反转*过滤器。默认情况下，过滤器都是包含型，显示为绿色，只有匹配过滤条件的结果才会显示。反转成排除型过滤器后，显示的是*不*匹配过滤器的检索项，显示为红色。

- 移除过滤器 ![img](images/11.png) 点击这个图标删除过滤器。

- 自定义过滤器 ![img](images/12.png) 点击这个图标会打开一个文本编辑框。编辑框内可以修改 JSON 形式的过滤器内容，并起一个 alias 别名： ![img](images/13.png)JSON 中可以灵活应用 bool query 组合各种 `should`、`must`、`must_not` 条件。一个用 `should`表达的 OR 关系过滤如下:
...
要查看一个文档的字段数据，点击表格第一列(通常都是时间)文档内容左侧的 `Expand` 按钮 ![img](images/6.jpg)。Kibana 从 Elasticsearch 读取数据然后在表格中显示文档字段。这个表格每行是一个字段的名字、过滤器按钮和字段的值。

![img](images/14.png)

1. 要查看原始 JSON 文档(格式美化过的)，点击 **JSON** 标签。
2. 要在单独的页面上查看文档内容，点击链接。你可以添加书签或者分享这个链接，以直接访问这条特定文档。
3. 收回文档细节，点击 **Collapse** 按钮 ![img](images/15.jpg)。
4. To toggle a particular field’s column in the Documents table, click the ![img](images/16.png) **Toggle column in table**button.
...
2. 从文档的字段数据添加一列，展开对应的文档后点击字段的![img](images/16.png)`Toggle column in table`按钮
...
1. 移动鼠标到字段列表的 `Selected Fields` 区域里你想要移除的字段上，然后点击它的 `remove` 按钮 ![img](images/18.jpg)。
